...
import { AlertTriangle, Check, Clock, Download, Loader2, RotateCcw, Sparkles, Wand2 } from 'lucide-react';
          ...
          {done && job?.result ? (
            ...
            <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[300px_1fr] lg:gap-12">
              ...
              <div className="flex flex-col justify-center">
                ...
                {/* Title and its Arabic name read as one unit. The Arabic used
                    to be a BLOCK with dir="rtl", which makes text-align right
                    by default — so it flew to the far edge of a wide column and
                    read as a stray element. As an inline span inside a normal
                    block it keeps correct bidi shaping and still starts where
                    the title starts, in either page direction. */}
                <h3 className="mt-4 font-display text-3xl font-bold leading-tight text-ink">
                  {job.result.surahEnglishName}
                ...
                <p className="mt-1.5">
                  <span dir="rtl" lang="ar" className="font-quran text-2xl text-accent">
                    {job.result.surahName}
                  </span>
                </p>

                {/* Three facts, three columns. Two columns left "size" orphaned
                    on a second row with an empty cell beside it. */}
                {/* The reciter's name is several times longer than a duration
                    or a file size, so equal thirds clipped it to "Mahmoud
                    Khalil …". It gets the extra width, and wraps rather than
                    truncates — reciter names vary and some locales are longer
                    still, so there is no column width that makes clipping safe. */}
                <dl className="mt-7 grid max-w-xl grid-cols-[1.6fr_1fr_1fr] items-start divide-x divide-border rounded-2xl border border-border bg-base-2/40 rtl:divide-x-reverse">
                  {[
                    ...
                    <div key={k} className="min-w-0 px-4 py-3">
                      <dt className="text-[0.65rem] font-semibold uppercase tracking-wider text-ink-3">{k}</dt>
                      <dd className="mt-1 text-[0.9rem] font-semibold leading-snug text-ink">{v}</dd>
                    </div>
                  ))}
                </dl>
                ...
                {/* A quiet note, not an alarm. Nothing has gone wrong — the
                    file is simply not kept — and a full-width amber panel with
                    a warning triangle was the loudest thing on a screen whose
                    job is to say "here is your reel". */}
                {job.expiresInMinutes !== undefined && (
                  <p className="mt-5 flex items-center gap-2 text-[0.8rem] text-ink-3">
                    <Clock className="h-3.5 w-3.5 shrink-0" />
                    {fill(t('expiresIn'), {
                      n: job.expiresInMinutes,
                      g: meta?.limits.downloadGraceMinutes ?? 3,
                    })}
                  </p>
                )}
              </div>
                  ...
                  <>
                    ...
                    <Fieldset title={t('optionsTitle')}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {/* Both of these were controls that made you decide
                            something before you had any reason to: a free-text
                            box whose correct answer is usually "nothing", and a
                            two-option dropdown. As toggles they state the
                            default and only ask for more when you say yes. */}
                        <ToggleChip
                          checked={customBg}
                          onChange={(v) => {
                            setCustomBg(v);
                            if (!v) set('backgroundKeywords', '');
                          }}
                          label={t('optCustomBackground')}
                        />
                        <ToggleChip
                          checked={form.basmala === 'always'}
                          onChange={(v) => set('basmala', v ? 'always' : 'off')}
                          label={t('optBasmalaAlways')}
                        />
                      </div>
                      ...
                      {customBg && (
                        <div className="mt-3 max-w-[22rem]">
                          <TextInput
                            id="keywords" maxLength={60} autoFocus
                            placeholder={t('keywordsPlaceholder')}
                            aria-label={t('fieldKeywords')}
                            value={form.backgroundKeywords}
                            onChange={(e) => set('backgroundKeywords', e.target.value)}
                          />
                          <p className="mt-1.5 text-[0.7rem] leading-snug text-ink-3">{t('keywordsHelpOn')}</p>
                        </div>
                      )}
                    </Fieldset>
                  </>
