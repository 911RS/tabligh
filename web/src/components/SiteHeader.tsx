        ...
        <nav className="hidden min-w-0 items-center lg:flex">
          {LINKS.map((l) => {
            ...
            return (
              ...
              >
                {/* The pill sits at z-0 with the label lifted above it, NOT at
                    -z-10. The bar is frosted glass — a translucent white layer
                    over a backdrop-filter — and a negative z-index put the pill
                    UNDERNEATH that layer, so the highlight was the brand orange
                    seen through about 65% white: it measured (250,210,186)
                    against a declared (249,115,22). The colour was never wrong,
                    it was just behind the glass. */}
                {on && (
                  <motion.span
                    layoutId="nav-pill"
                    className="absolute inset-0 z-0 rounded-full bg-gradient-to-b from-accent-lit to-accent shadow-accent"
