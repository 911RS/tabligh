...
function save(){const g=id=>document.getElementById(id),ids=id=>g(id).value.split(',').map(x=>x.trim()).filter(Boolean);
  const patch={schedule:{enabled:g('sch_en').checked,tz:g('sch_tz').value,times:getTimes()},content:{translationEdition:g('c_tr').value,randomMinAyahs:+g('c_min').value,randomMaxAyahs:+g('c_max').value},branding:{karaokeEnabled:g('b_kar').checked,watermarkEnabled:g('b_wm').checked,textFillColor:g('b_fill').value,watermarkHandle:g('b_h').value,outroText:g('b_out').value},publish:{channels:{tiktok:ids('p_tiktok'),instagram:ids('p_instagram'),facebook:ids('p_facebook'),youtube:ids('p_youtube')}}};
